...
- Create a strategy function, following the same format as the boilerplate strategy. This should take in an :py:class:`Exchange <prototrade.exchange.exchange.Exchange>` object as its first argument. 
The :py:class:`Exchange <prototrade.exchange.exchange.Exchange>` is how a strategy can retrieve the latest quotes for subscribed stocks and place orders for symbols. 
...
The Exchange Class
------------------
The :py:class:`Exchange <prototrade.exchange.exchange.Exchange>` argument to each strategy function is the primary way of interacting with the Prototrade framework.

- Submit orders to the framework using 
- Cancel orders using 
